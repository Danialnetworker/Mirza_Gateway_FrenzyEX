...
<code>$subscriptionurl</code>";
    ...
    step('get_step_payment', $from_id);
} elseif ($user['step'] == "get_step_payment" && in_array($datain, ["cart_to_offline", "aqayepardakht", "zarinpal", "plisio", "nowpayment", "iranpay1", "iranpay2", "iranpay4", "frenzyex", "iranpay3", "digitaltron", "startelegrams"])) {
    ...
    // method clause a buyer aims this card-to-card receipt flow at an order
    // of another gateway -- the cooldown query above only looks at 'cart to
    // cart', so it does not see it either -- and puts a Confirm_pay_ button
    // for it in front of an admin who is looking at what reads as an
    // ordinary receipt.
